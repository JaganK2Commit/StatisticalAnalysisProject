...
author: "STAT 420, Summer 2021. Team  42 PST: Jagadeesh Kedarisetty (jk64), Nilesh Bhandarwar (nileshb2), Peri Rocha (procha2)"
date: "08/08/2021"
output:
  html_document: 
    theme: readable
...
Those variables contain the same value almost 99.84% of the observations. Therefore, we'll eliminate the `garageSpaces` variable from the dataset. 
...
When we compare the model with all predictor versus one that does not include the hasGarage predictor, we see the p-value significant at 0.94, so we fail to reject the null hypothesis. Therefore, we'll remove the hasGarage predictor from the analysis. 
...
housing_data_model_significant =  lm(latestPrice ~ zipcode+longitude+propertyTaxRate+hasAssociation+yearBuilt+numPriceChanges+latest_saleyear+numOfPatioAndPorchFeatures+lotSizeSqFt+livingAreaSqFt+numOfPrimarySchools+numOfElementarySchools+numOfHighSchools+avgSchoolDistance+avgSchoolRating+avgSchoolSize+numOfBathrooms+numOfBedrooms, data = housing_data_train)

anova(housing_data_model, housing_data_model_significant) # 0.000057
...
From the $R^2$ value, about 55.69% of data is explained by this model, with 46 predictors. Next, we will try to find a “better” model with $R^2$ greater than 0.5569, or adjusted $R^2$  greater than 0.5509. 

**TO DO: LOOCV_RMSE CALCULATION NOT WORKING** We'll also try to lower the value of LOOCV_RMSE (< 111435) to explain the data.

Next, we'll investigate how well backwards AIC and BIC performs in the additive model.
...
The adjusted $R^2$ values are 0.5515 for the AIC model, and 0.5467 for the BIC model. Both of them are inferior to the additive model before backwards AIC and BIC. 
...
anova(housing_data_model, housing_data_model_leaps_best)
```

The leaps model has a p-value < 2e-16 and therefore can be considered.
...
From the $R^2$ value, about 58.99% of data is explained by this model, with 56 predictors. The adjusted $R^2$  is 0.5836. This model is preferred over the addictive model. 
...
**TO DO: NEED COMMENTS ON THE CHARTS**
...
**TO DO: NEED COMMENTS ON THE IMPROVED CHARTS**
...
As seen in box cox transformation, we can see that the mean of the normal distribution is centered around 0.43. We tried using 0.43 to 0.50 as exponential formula as per the box cox transformation, and we got best model at 0.50. Our p-value for normal distribution is 0.4, however, constant variation is still showing a low p-value. As we improved a lot based on the original Fitted versus Residual plot, we are choosing the **interactive model with backwards AIC without outliers (housing_data_model_interaction_aic_without_outliers)** as our final model after applying box cox transformation. 
...
error
```
<br/> We are calculating error/noise with final model for test and train. We need to reverse transformation (Boxcox applied on model) on sigma of model to get final error value. These small error value shows greater accuracy of model. <br/>
=======
...
predictions_train = predict(housing_data_model_interaction_aic_without_outliers, housing_data_train)
rmse(housing_data_train$latestPrice, predictions_train)


predictions_test = predict(housing_data_model_interaction_aic_without_outliers, housing_data_test)
rmse(housing_data_test$latestPrice, predictions_test)

```

>>>>>>> c072e8fc265aad3edef1da6e6e9d9a77bc69e6c1
# Links and citations
